...
def compute_plain_solver_stats(
    ...
    for data in data_list:
        ...
        if existing is not None and existing != num_samples:
            raise ValueError(
                f"compute_plain_solver_stats found conflicting sample counts for cnf_id={cnf_id}: "
    ...
    start_time = time.time()
    stats_dicts = Parallel(n_jobs=num_workers, prefer="threads")(
        delayed(solver_pool_fn)((cnf_id, 0, dataset.cnf_list[cnf_id], None, solver_params))
        for cnf_id in layout
    )
    end_time = time.time()
    print(f"Solved {len(layout)} unique plain formulas in {end_time - start_time:.2f} seconds")

    if not stats_dicts:
        return pd.DataFrame(columns=["cnf_id", "sample_id", "file"])
    rows = []
    for (cnf_id, num_samples), stats in zip(layout.items(), stats_dicts):
        for sample_id in range(num_samples):
            row = dict(stats)
            row["cnf_id"] = cnf_id
            row["sample_id"] = sample_id
            row["file"] = dataset.id_to_file[cnf_id]
            rows.append(row)
